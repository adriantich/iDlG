...
class CreatePlot:
    ...
    _cached_figs: dict[str, go.Figure] = {}
    ...
    def main(self):
        results_correct =self.get_results()
        if results_correct:
            self.fig = self.create_plot()
        else:
            self.fig = None

    def prepare_data(results: pd.DataFrame) -> pd.DataFrame:

        if results.empty:
            raise ValueError("No results available to prepare.")

        required_columns = {"start", "end"}
        missing_columns = required_columns - set(results.columns)
        if missing_columns:
            missing_display = ", ".join(sorted(missing_columns))
            raise ValueError(f"Results are missing required columns: {missing_display}.")

        results = results.sort_values("start").reset_index(drop=True)
    ...
    def get_results(self):
        if f"{self.parquet_path}" not in CreatePlot._cached_frames.keys():
            self.results = pd.read_parquet(self.parquet_path)
            if self.results.empty:
                print(f"No results found in the parquet file at {self.parquet_path}.")
                return False
            else:
                self.results = CreatePlot.prepare_data(self.results)
                CreatePlot._cached_frames[f"{self.parquet_path}"] = self.results
                return True
        else:
            self.results = CreatePlot._cached_frames[f"{self.parquet_path}"]
            return True
    ...
    def create_plot(self):
        ...
        for sample_name in self._sample_columns(self.results):
            # plt.scatter(
            #     self.results['midpoint'], 
            #     self.results[sample_name],
            #     c=cm.jet(self.results[sample_name]/2),
            #     edgecolor='none',
            # )
            plt.plot(
                self.results['midpoint'],
